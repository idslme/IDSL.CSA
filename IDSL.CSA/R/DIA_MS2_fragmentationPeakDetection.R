      ...
      if (!is.null(chromatogramMatrixMS1)) {
        ...
        if (!is.null(Segment)) {
          ...
          if (L_x_seg_apex > 0) {
            ...
            ##
            x_mz_MS2 <- which(peaks_MS2[, 2] >= intensityThresholdFragment)
            ...
            if (L_mz_MS2 > 1) {
              ...
              if (length(x_fragment_dia) > 1) {
                ...
                if (plotEICcheck) {
                  ...
                  plot(RT_chrom_MS1, Int_chrom_MS1, type = "l", ylim = c(0, yMaxLimPlot*1.01), lwd = 4, col = colors[1], cex = 4, xlab = "", ylab = "")
                  ##
                  pCounter <- 1
                  for (p in 1:(nLines - 1)) {
                    pCounter <- pCounter + 1
